...
const (
	...
	tenantDBIndexFilePath  = "../sql/tenant/create_index.sql"
...
func createTenantDB(id int64) error {
	...
	cmdIndex := exec.Command("sh", "-c", fmt.Sprintf("sqlite3 %s < %s", p, tenantDBIndexFilePath))
	if out, err := cmdIndex.CombinedOutput(); err != nil {
...
func dispenseID(ctx context.Context) (string, error) {
	...
	// for i := 0; i < 100; i++ {
	// 	var ret sql.Result
	// 	ret, err := adminDB.ExecContext(ctx, "REPLACE INTO id_generator (stub) VALUES (?);", "a") // slow query
	// 	if err != nil {
...
func playerHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(
		...
		"SELECT ps.* FROM player_score ps JOIN ("+
			// " SELECT competition_id, MAX(row_num) AS max_row_num FROM player_score"+
			" SELECT MAX(row_num) AS max_row_num FROM player_score"+
			" WHERE tenant_id = ? AND player_id = ? GROUP BY competition_id"+
			" ) latest"+
			" ON ps.row_num = latest.max_row_num",
		// " ON ps.competition_id = latest.competition_id AND ps.row_num = latest.max_row_num",
		v.tenantID,
		p.ID,
	); err != nil {
	...
	// for _, c := range cs {
	// 	ps := PlayerScoreRow{}
	// 	if err := tenantDB.GetContext( // CPU使用率が高い
	// 		ctx,
	// 		&ps,
	// 		// 最後にCSVに登場したスコアを採用する = row_numが一番大きいもの
