...
func dispenseID(ctx context.Context) (string, error) {
	...
	for i := 0; i < 100; i++ {
		var ret sql.Result
		ret, err := adminDB.ExecContext(ctx, "REPLACE INTO id_generator (stub) VALUES (?);", "a") // FIXME: slow, many calls
		if err != nil {
...
func playerHandler(c echo.Context) error { // FIXME: many calls
	...
	if err := tenantDB.SelectContext(
		ctx,
		&pss,
		"SELECT * FROM player_score where player_id = ? group by tenant_id, competition_id HAVING MAX(row_num)",
		p.ID,
	); err != nil {
		// 行がない = スコアが記録されてない
